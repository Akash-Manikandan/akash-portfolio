...
const ProfessionalCard = ({ experience, className = '' }: { experience: ProfessionalCardProps, className: string }) => {
    ...
    return (
        <Card className={cn(className)}>
            <CardHeader className={"flex justify-between flex-row gap-2 max-sm:flex-col"}>
                <h3 className={cn(lora.className, "text-2xl font-semibold")}>{experience.companyName}</h3>
            ...
            <CardContent className="flex flex-col">
                ...
                {(experience.position.length > 0) && (<CollapsibleList list={experience.position} />)}
